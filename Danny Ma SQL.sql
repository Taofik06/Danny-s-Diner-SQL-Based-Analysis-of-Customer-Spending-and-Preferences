...
WITH cte AS 
	(SELECT *,
	ROW_NUMBER() OVER(PARTITION BY customer_id ORDER BY order_date) AS cust_rank 
FROM sales LEFT JOIN menu USING(product_id))
SELECT customer_id, product_name FROM cte WHERE cust_rank = 1;

-- question 3 can also be answered with
WITH cte AS 
	(SELECT customer_id, min(order_date) AS order_date 
    FROM sales 
    GROUP BY customer_id)
SELECT c.customer_id, c.order_date, product_name 
FROM cte c JOIN sales s 
	ON s.customer_id = c.customer_id 
	AND c.order_date = s.order_date
JOIN menu m ON s.product_id = m.product_id
GROUP BY customer_id, product_name;


-- 4. What is the most purchased item on the menu and how many times was it purchased by all customers?
SELECT product_name, count(*) no_of_sales 
FROM sales s LEFT JOIN menu m 
ON s.product_id = m.product_id #
...
WITH  cte AS 
	(SELECT 
		...
		product_name, 
		order_date,
		ROW_NUMBER() OVER(PARTITION BY customer_id ORDER BY order_date) AS cust_rnk
...
-- question 6 can also be answered with the query below:
SELECT 
	s.customer_id, 
    product_name, 
...
WHERE s.order_date >= m.join_date
GROUP BY customer_id, product_name
ORDER BY customer_id, product_name, order_date
LIMIT 3;
...
-- Question 7 can also be answered with:
SELECT 
	s.customer_id, 
    product_name, 
